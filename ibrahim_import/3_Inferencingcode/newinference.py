...
# =============================================================================
# Graph construction (same semantics as training)
# =============================================================================
...
def validate_against_fluent_wall_csv(
    wall_csv_path: Path,
    wall_pos_xyz: np.ndarray,
    pred_wss_vec: np.ndarray,
    ...
    cfd_node_wss = (sum_wss / np.where(cnt > 0, cnt, 1.0)).astype(np.float64)
    hit_counts = cnt[:, 0].astype(np.int64)

    pred = pred_wss_vec.astype(np.float64)
    err_vec = pred - cfd_node_wss
    err_mag = np.linalg.norm(err_vec, axis=1)
    pred_mag = np.linalg.norm(pred, axis=1)
    cfd_mag = np.linalg.norm(cfd_node_wss, axis=1)

    has_cfd = hit_counts > 0
    mask_rel = has_cfd & (cfd_mag > float(rel_err_mask_min_cfd_mag))
    eps = 1e-12
    rel_err = np.full_like(cfd_mag, np.nan, dtype=np.float64)
    rel_err[mask_rel] = err_mag[mask_rel] / (cfd_mag[mask_rel] + eps)

    mask_corr = mask_rel
    if np.sum(mask_corr) >= 3:
        x = cfd_mag[mask_corr]
        y = pred_mag[mask_corr]
    ...
    mse_vec_hit = float(np.mean(err_vec[has_cfd] ** 2)) if np.any(has_cfd) else float("nan")
    mae_mag_all = float(np.mean(err_mag))
    mae_mag_hit = float(np.mean(err_mag[has_cfd])) if np.any(has_cfd) else float("nan")
    ...
    metrics = {
        ...
        "nn_hits_nonzero_frac": float(np.mean(has_cfd)),
        "mse_vec_all_nodes": mse_vec_all,
        "mse_vec_hit_nodes": mse_vec_hit,
        "mae_mag_all_nodes": mae_mag_all,
        "mae_mag_hit_nodes": mae_mag_hit,
        "rel_err_mean_masked": rel_err_mean_masked,
        "rel_err_median_masked": rel_err_median_masked,
        "pearson_r_mag_masked": r,
        "r2_mag_masked": r2,
    }
    ...
    extra_csv_cols = {
        ...
        "ERRmag": err_mag,
        "NN_hit_count": hit_counts.astype(np.float64),
        "NN_dist_csv_rows_pooled": np.full((Nw,), np.nan, dtype=np.float64),
    }
    map_info = {
...
def export_vtp_meshio(out_vtp: Path, pos_xyz: np.ndarray, faces_local: np.ndarray, point_data: Dict[str, np.ndarray]) -> bool:
    try:
    ...
    cells = [("triangle", np.asarray(faces_local, dtype=np.int32))]
    mesh = meshio.Mesh(points=np.asarray(pos_xyz, dtype=np.float64), cells=cells, point_data=point_data)
    mesh.write(out_vtp, file_format="vtp")
    return True
...
def main() -> None:
    ...
    try:
        ...
        data, ginfo = build_wall_graph_from_faces(nodes_xyz, faces_by_zone, wall_zones)
        ...
        if args.cfd_wall_csv:
            ...
            logger.log("=" * 96)
            logger.log("VALIDATION vs Fluent wall CSV (nearest-neighbor mapped CSV rows -> wall nodes)")
            logger.log("=" * 96)
            for k, v in validation_metrics.items():
                logger.log(f"  {k:28s}: {v}")
        ...
        if args.export_vtp:
            ...
            ok = export_vtp_meshio(out_vtp, pos_np, faces_local, point_data=point_data)
            vtp_written = bool(ok)
            if ok:
        ...
        if args.save_json or do_any_output:
            meta = {
                ...
                "validation_map_info_summary": {
                    ...
                    "nn_dist_mean": float(np.mean(validation_map_info["nn_dist_csv_to_node"])) if validation_map_info else None,
                    "nn_dist_p99": float(np.quantile(validation_map_info["nn_dist_csv_to_node"], 0.99)) if validation_map_info else None,
